...
namespace custom
{
        ...
        public sealed class SkillState : SkillFile {
            ...
            public SkillState(Dictionary<string, AbstractStoragePool> poolByName, StringPool strings, StringType stringType,
                ...
                try {
                    ...
                    poolByName.TryGetValue("custom", out p);
                    CustomsField = (null == p) ? (P0)Parser.newPool("custom", null, types) : (P0) p;
            ...
            internal readonly P0 CustomsField;

            public override P0 Customs() {
                return CustomsField;
            }
        }
    ...
    /// </summary>
    public sealed class P0 : BasePool<custom.Custom> {
        
            protected override custom.Custom[] newArray(int size) {
                return new custom.Custom[size];
            }

            /// <summary>
            /// Can only be constructed by the SkillFile in this package.
            /// </summary>
            internal P0(int poolIndex) : base(poolIndex, "custom", noKnownFields, NoAutoFields) {

            }

            internal custom.Custom[] Data {
                get
                {
                    return (custom.Custom[])data;
                }
            }
            ...
            public override void allocateInstances(Block last) {
                ...
                while (i < high) {
                    data[i] = new custom.Custom(i + 1);
                    i += 1;
                }
            }
        

            /// <returns> a new Custom instance with default field values </returns>
            public override object make() {
                custom.Custom rval = new custom.Custom();
                add(rval);
                return rval;
            }
        
            public CustomBuilder build() {
                return new CustomBuilder(this, new custom.Custom());
            }

            /// <summary>
            /// Builder for new Custom instances.
            ///
            /// @author Simon Glaub, Timm Felden
            /// </summary>
            public sealed class CustomBuilder : Builder<custom.Custom> {

                public CustomBuilder(AbstractStoragePool pool, custom.Custom instance) : base(pool, instance) {

                }

                public override custom.Custom make() {
                    pool.add(instance);
                    custom.Custom rval = instance;
                    instance = null;
                    return rval;
                }
            }
            ...
            private sealed class UnknownSubPool : StoragePool<custom.Custom.SubType, custom.Custom> {
                ...
                public override void allocateInstances(Block last) {
                    ...
                    int high = (int)(i + last.staticCount);
                    while (i < high) {
                        data[i] = new custom.Custom.SubType(this, i + 1);
                        i += 1;
                    }
                }
            }
